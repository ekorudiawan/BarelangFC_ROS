...
function arm(deg1, deg2, deg3, hard)
  Body.set_larm_hardness(hard);
  Body.set_rarm_hardness(hard);
  qLArm=math.pi/180*vector.new({deg1,deg2,deg3});
  qRArm=math.pi/180*vector.new({deg1,-deg2,deg3});
  Body.set_larm_command(qLArm);
  Body.set_rarm_command(qRArm);
end

function rad(angle) --radianTodegree
  return angle*180/math.pi;
end

function deg(angle) --degreeToradian
  return angle*180/math.pi;
end
...
--    print("\nHead Yaw Pitch:", unpack(headangle*180/math.pi))
    print("\nHead Yaw Pitch:", unpack(headangle))
  end
end
